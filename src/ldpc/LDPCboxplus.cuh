...
namespace{
	...
	template<std::floating_point T>
	class device_sum_accumlator{
		T abssum;
		uint_of_length_t<T> signprod;
	public:
		device_sum_accumlator():abssum(0),signprod(0){}
		void operator+=(T rhs);
		T operator-(T rhs) const;
	};
	...
	template<std::floating_point T>
	class device_min_accumlator{
		std::pair<T,T> absmin;
		uint_of_length_t<T> signprod;
	public:
		device_min_accumlator():absmin(std::numeric_limits<T>::infinity(),std::numeric_limits<T>::infinity()),signprod(0){}
		void operator+=(T rhs);
		T operator-(T rhs) const;
	};
}
