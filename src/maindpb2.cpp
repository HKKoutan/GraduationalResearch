...
int main(int argc, char* argv[]){
	...
	string path2;
	if(!util::search_txt_to_str("i2.txt",path,path2)){
	...
	code::Systematic_LDPC ldpc2(path2, NUM_THREADS);

	tk.split();

	for(uint32_t t=0; t<NUM_THREADS; t++){
		threads.emplace_back([&ldpc, &ldpc2, &biterrors, &bitcounts, &nterrors, &ntcounts, &GCpercentages, &noise_factor, repeat_per_thread, t](){
			for(uint32_t n=0; n<noise_factor.size(); n++){
				...
				for(uint32_t r=0; r<repeat_per_thread; r++){
					...
					iest = ldpc2.sumproduct_decode(LLRi,t);
