...
template<std::uint8_t ATGC=0x1B, std::floating_point FPTYPE = float>
...
template<>
auto Nanopore_Sequencing<0x1B,float>::condprob_init() const{
...
template<>
auto Nanopore_Sequencing<0x27,float>::condprob_init() const{
	return std::array<std::array<double,4>,4>{
...
template<std::uint8_t ATGC>
Nanopore_Sequencing<ATGC,float>::Nanopore_Sequencing(double alpha) : Nanopore_Sequencing(alpha, 0){}

template<std::uint8_t ATGC>
Nanopore_Sequencing<ATGC,float>::Nanopore_Sequencing(double alpha, std::int64_t seed):
	mt(seed),
	uniform(0,1),
...
template<std::uint8_t ATGC>
template<std::size_t L>
auto Nanopore_Sequencing<ATGC,float>::noise(const std::array<code::DNAS::nucleotide_t<ATGC>,L> &in){
	auto out = in;
	for(auto &i: out){
