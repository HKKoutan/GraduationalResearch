...
constexpr size_t R_SOURCE_LENGTH = 256u;
constexpr size_t R_CODE_LENGTH = 512u;
...
int main(int argc, char* argv[]){
	...
	code::Systematic_LDPC<R_SOURCE_LENGTH,R_CODE_LENGTH> ldpcr;

	tk.split();

	for(uint32_t t=0; t<NUM_THREADS; t++){
		threads.emplace_back([&ldpc, &ldpcr, &biterrors, &bitcounts, &nterrors, &ntcounts, &GCpercentages, &noise_factor, repeat_per_thread, t](){
			for(uint32_t n=0; n<noise_factor.size(); n++){
				...
				for(uint32_t r=0; r<repeat_per_thread; r++){
					...
					ir = ldpcr.encode(info);
					info.insert(info.end(),ir.begin(),ir.end());
					...
					ch.redundancy_LLR(ci,LLRi,crbar[crbar.size()-1]);
					iest = ldpcr.sumproduct_decode(LLRi,t);

					code::DNAS::nt_addequalizing_decode(crbar,iest,cr);
